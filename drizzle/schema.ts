 ...
 */
export const serverServices = mysqlTable("server_services", {
  id: int("id").autoincrement().primaryKey(),
  serviceId: varchar("serviceId", { length: 50 }).notNull(), // ID del servicio en UnlockerFast
  serviceName: varchar("serviceName", { length: 300 }).notNull(),
  groupName: varchar("groupName", { length: 200 }),
  serviceType: mysqlEnum("serviceType", ["IMEI", "SERVER", "REMOTE"]).notNull(),
  precio: decimal("precio", { precision: 10, scale: 2 }).notNull(), // Precio de costo
  precioVenta: decimal("precioVenta", { precision: 10, scale: 2 }).notNull(), // Precio de venta al cliente
  tiempo: varchar("tiempo", { length: 100 }), // Tiempo estimado
  info: text("info"), // Descripción del servicio
  activo: int("activo").default(1).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});

export type ServerService = typeof serverServices.$inferSelect;
export type InsertServerService = typeof serverServices.$inferInsert;

/**
 * Pedidos de Servicios de Servidor
 */
export const serverOrders = mysqlTable("server_orders", {
  id: int("id").autoincrement().primaryKey(),
  codigo: varchar("codigo", { length: 50 }).notNull().unique(), // Código único del pedido (ORD-XXXX)
  serviceId: int("serviceId").notNull(), // FK a serverServices
  cliente: varchar("cliente", { length: 200 }),
  telefono: varchar("telefono", { length: 50 }),
  email: varchar("email", { length: 320 }),
  imei: varchar("imei", { length: 20 }), // Para servicios IMEI
  customFields: text("customFields"), // JSON con campos personalizados
  precioVenta: decimal("precioVenta", { precision: 10, scale: 2 }).notNull(),
  precioCosto: decimal("precioCosto", { precision: 10, scale: 2 }).notNull(),
  ganancia: decimal("ganancia", { precision: 10, scale: 2 }).notNull(),
  estado: mysqlEnum("estado", ["pendiente", "procesando", "completado", "fallido", "cancelado"]).default("pendiente").notNull(),
  referenceId: varchar("referenceId", { length: 100 }), // ID de referencia de UnlockerFast
  resultado: text("resultado"), // Resultado del servicio (código, archivo, etc.)
  notas: text("notas"),
  tienda: mysqlEnum("tienda", ["admin", "sucursal"]).default("admin").notNull(),
  fechaPedido: timestamp("fechaPedido").defaultNow().notNull(),
  fechaCompletado: timestamp("fechaCompletado"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});

export type ServerOrder = typeof serverOrders.$inferSelect;
export type InsertServerOrder = typeof serverOrders.$inferInsert;
