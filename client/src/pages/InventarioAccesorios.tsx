            ...
            return (
              <Card key={accessory.id} className={`p-4 ${stockBajo ? 'border-yellow-300 bg-yellow-50' : ''}`}>
                ...
                <div className="flex items-start gap-3 mb-3">
                  <div
                    className="w-16 h-16 rounded-lg overflow-hidden bg-white flex-shrink-0 flex items-center justify-center cursor-pointer border border-gray-200 hover:border-purple-400 transition-colors group"
                    onClick={() => setImagePickerAccessory(accessory)}
                    title="Cambiar imagen"
                  >
                    {accessory.imagen ? (
                      <img src={accessory.imagen} alt={accessory.nombre} className="w-full h-full object-contain p-1" />
                    ) : (
                      <div className="flex flex-col items-center gap-0.5 text-gray-400 group-hover:text-purple-500 transition-colors">
                        <ImageIcon size={16} />
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1 mb-1">
                      <span className="text-xs font-mono text-gray-500">{accessory.codigo}</span>
                      {stockBajo && <AlertCircle className="h-4 w-4 text-yellow-600 flex-shrink-0" />}
                    </div>
                    <h3 className="font-semibold text-sm leading-tight line-clamp-3">{accessory.nombre}</h3>
                  </div>
                </div>
